...
- **Adaptive FPS / resolution** — Quality already adapts per frame, but capture
  cadence and resolution are still bounded by startup settings. Back off FPS or
  scale when WebSocket writes or encode work show sustained congestion, then
  recover when the path clears.
